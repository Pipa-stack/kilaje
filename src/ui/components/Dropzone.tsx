    ...
    <label
      ...
      className={[
        ...
        dragging
          ...
          : 'border-iron-700 bg-iron-900 hover:border-iron-600 hover:bg-iron-850',
        disabled ? 'cursor-progress opacity-60' : '',
      ].join(' ')}
    >
      <Icon name="upload" size={28} className="text-iron-400" />
      <span className="text-base font-semibold text-chalk">{label}</span>
      {hint ? <span className="max-w-xs text-sm text-iron-400">{hint}</span> : null}
      <input
        id={inputId}
        type="file"
